...
export function Footer() {
  return (
    <footer className="border-line-soft/70 border-t px-5 py-14 sm:px-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-10">
          ...
          <nav className="flex flex-wrap gap-x-8 gap-y-3">
            {LINKS.map((l) => (
              <a
                key={l.label}
                href={l.href}
                className="hover:text-lime text-base font-semibold text-white/60 transition-colors"
              >
                {l.label}
              </a>
            ))}
        ...
        <p className="max-w-3xl text-sm leading-relaxed text-white/35">
